    ...
    override fun onCreateView(
        ...
        binding.eventCalendar.setOnDateChangeListener { calendarView, i, i2, i3 ->
            Log.d("SECONDFRAG", calendarView.date.toString())
             sot= i.toString()+"/"+(i2+1).toString()+"/"+i3.toString()
        }
       binding.doneBtn.setOnClickListener{

               val kotEvent= Event(binding.eventTitleEt.text.toString(),binding.eventCategoryEt.text.toString(),sot)
               requireActivity().supportFragmentManager.beginTransaction()
                   .replace(R.id.main_frag_container,FirstFragment.newInstance(kotEvent))
                   .commit()
